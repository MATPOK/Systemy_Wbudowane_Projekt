...
Chcąc zachować spójność z prawdziwym procesem produkcyjnym, kaskada została zaprojektowana tak, aby w jak najwyższym stopniu odzwierciedlać rzeczywiste proporcje. Baza i utwardzacz są napełniane, podgrzewane i utrzymywane w gotowości zgodnie z wymaganiami technologicznymi. Mieszanie odbywa się przez określoną liczbę cykli, a temperatura gotowej mieszaniny jest kontrolowana, aby zapewnić odpowiednią jakość lakieru. System jest również wyposażony w zabezpieczenia, które chronią przed przelaniem, przegrzaniem oraz pracą elementów wykonawczych bez cieczy w zbiornikach. Oczywiście ze względu na sposób realizacji projektu, pewne aspekty procesu produkcji lakierów zostały uproszczone np. jeden zbiornik z bazą niezależnie od typu lakieru.\\
Poniższe zdjęcia z kart katalogowych producentów lakierów ilustrują proporcje mieszania:
\begin{figure}[H]
    \centering
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item $h \ge x2$ Grzej G1 do temperatury T1, a następnie przez dokładnie 160 cykli\\
        Oczekuj na opróżnienie zbiornika do poziomu x1, podtrzymując temperaturę T1
    \end{enumerate}
    \item Zbiornik nr 2 - utwardzacz
    \begin{enumerate}
        \item $h < x3$ Napełniaj Z2 do poziomu x3
        \item $x3 \leq h < x4$ Dalej napełniaj Z2 do poziomu x4, jednocześnie grzejąc G2
        \item $h \ge x4$ Grzej G2 do temperatury T2, a następnie przez dokładnie 160 cykli\\
    ...
    \item Zbiornik nr 3 - mieszanina
        ...
        \end{itemize}
         \item Tryb manualny
         \begin{itemize}
            \item W każdym momencie poprzez K8, użytkownik może przejść do trybu ręcznego
            \item W momencie przejścia do trybu manualnego, wszystkie wyjścia zbiorników zostają wyłączone, a poprzez K1, K2 i K3 użytkownik może ręcznie sterować napełnianiem, grzaniem i mieszaniem
...
\begin{lstlisting}[language=C, caption={Tryb automatyczny}, label={Kod:Auto}]
    /*Blok kodu Manualnego (poprzedni listing)*/
       ...
       // Wyświetlanie informacji na LCD
        if (stan3 == 8 || stan3 == 9) {
            sprintf(bufor_pomocniczy, "AUTO: ZEROWANIE");  // Trwa opróżnianie po powrocie z pracy manualnej
...
\begin{lstlisting}[language=C, caption={Pełny kod źródłowy projektu}, label={Kod:calosc}]
   /*************************************************************************/
/* Program testowy dla obiektu A - AUTO + MANUAL Z ZABEZPIECZENIAMI      */
/* Wersja ulepszona (Industrial Standard / Moore Machine)                */
/*************************************************************************/
#include "spsym.h"
#include <stdio.h>
...
char stan1, stan2, stan3; 
int tryb, nastepny_tryb;

float temp1, temp2, temp3;
float T1_set, T2_set, T3_set;

#define T1 (temp1 >= T1_set)
#define T2 (temp2 >= T2_set)
#define T3 (temp3 >= T3_set)
...
void prolog(void) {
    ...
    tim1 = 0; tim2 = 0; tim3 = 0;
    stan1 = 0; stan2 = 0; stan3 = 8; 
    tryb = 0; nastepny_tryb = 1;
    T1_set = 30.0; T2_set = 30.0; T3_set = 45.0;
...
void oblicz(void) {
    ...
    if (KL && !prev_KL) {
        for(i = 0; i < 10; i++) man_out[i] = 0;
        stan1 = 0; stan2 = 0; stan3 = 8; tryb = 0; 
    }
    ...
    if (KL) {
        ...
        L1=aK1; L2=aK2; L3=aK3; L4=0; L5=0; L6=0; L7=0;
        ...
        if (stan3 == 2 || stan3 == 4) { timer_pauzy++; if (timer_pauzy > 600) timer_pauzy = 0; } 
        else { timer_pauzy = 0; }
        ...
        switch (stan3) {
            ...
            case 1: 
                ...
                if (x7) { tim3 = 300; stan3 = 5; } 
                    ...
                    tim3 = (tryb == 1) ? 200 : 100; // Akryl 2s, Poliuretan 1s
                    stan3 = 3; 
                } 
                break;
            case 2: 
                Z2=0; Z4=0; Z5=0; M=0; G3=(timer_pauzy <= 300) ? 1 : 0; G4=0; 
                pauza_tim3 = 1; // ZATRZYMANIE TIMERA
                if (x1 && T1) stan3 = 1; 
                break;
            case 3: 
                Z2=0; Z4=1; Z5=0; M=0; G3=0; G4=0; 
        ...
        //WYSWIETLACZ W TRYBIE AUTO 
        if (stan3 == 8 || stan3 == 9) {
            sprintf(bufor_pomocniczy, "AUTO: ZEROWANIE"); 
        } else if (stan3 == 0 || stan3 == 2 || stan3 == 4) { 
            sprintf(bufor_pomocniczy, "AUTO: UZUPELNIANIE"); 
        } else if (tryb == 1) {
            sprintf(bufor_pomocniczy, "AUTO: AKRYL"); 
        } else if (tryb == 2) {
            sprintf(bufor_pomocniczy, "AUTO: POLIURETAN"); 
        } else {
            sprintf(bufor_pomocniczy, "AUTO: CZEKA"); 
        }
        sprintf(buf, "%-20s", bufor_pomocniczy);
        LCD_xy(1,1); LCD_puts(buf);
...
void przerwanie(void) { licz++; }
void komunikacja(void) {}

#ifdef Symulator_PC
void wykres(void) {
    aTab[0] = (int)(AI[0]/10.0); aTab[1] = (int)(AI[1]/10.0); aTab[2] = (int)(AI[2]/10.0);
    aTab[3] = (int)(AI[3]/10.0); aTab[4] = (int)(AI[4]/10.0); aTab[5] = (int)(AI[5]/10.0);
    aTab[6] = (int)(licz/100.0); aTab[7] = AO[7];

    bTab[0] = (int)aK1; bTab[1] = (int)aK2; bTab[2] = (int)L1; bTab[3] = (int)L2;
    bTab[4] = BI[0].d&0x04; bTab[5] = BI[0].d&0x02; bTab[6] = BI[0].d&0x01; bTab[7] = BI[0].d&0x80;
    bTab[8] = Z3; bTab[9] = G1; bTab[10] = M; bTab[11] = 11;
    bTab[12] = 12; bTab[13] = 13; bTab[14] = 14; bTab[15] = 15;
}
#endif
\end{lstlisting}

\section{Przedstawienie działania programu}
Pod podanym linkiem znajduje się \href{https://youtu.be/baM-8L8GW4s} {nagranie wideo prezentujące działanie programu}.\\
...
\begin{figure}[H]
    ...
    \caption{Tryb automatyczny - produkcja lakieru akrylowego, pauza z powodu uzupełniania braków w zbiorniku nr 1}
